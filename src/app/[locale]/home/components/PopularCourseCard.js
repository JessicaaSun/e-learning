...
const PopularCourseCard = ({
  ...
  return (
    <div className="rounded-[25px] bg-gradient-primary-to-right ">
      
    <div
      style={{ filter: "drop-shadow(0px 8px 10px rgba(0, 0, 0, 0.06))" }}
      className=" relative sm:px-10 sm:py-5 flex max-sm:flex-col md:flex-row items-center bg-white rounded-[25px] transform hover:-translate-y-3  transition-all"
    >
      <Image
      ...
      <div className="ps-10 lg:w-[750px] md:w-[600px] sm:w-[500px]">
        ...
        <button
          ...
          className={` text-base mt-5  text-white flex ${btnColor}  focus:ring-4 focus:outline-none  rounded-3xl px-7 py-2  text-center `}
        >
          <Link href={`/courses/${title.toLowerCase()}`}>
            {t("Start Learning")}
          </Link>
        </button>
        <div className="flex mt-4 justify-between">
          ...
          <div className="flex ">
            ...
            <p className="ps-2"> {requiredTime} {t("hours")}</p>
          </div>
          <div className="flex ">
            <svg
              width="23"
            ...
            <p className="ps-2">{lesson} {t("lessons")}</p>
          </div>
        </div>
      </div>
    </div>
